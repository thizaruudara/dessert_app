  ...
  Future<void> _checkForNewScheduledPapers(List<PaperSession> sessions, String userExamYear) async {
    ...
    try {
      ...
      for (final session in sessions) {
        ...
        final key = 'seen_paper_push_${session.id}';
        ...
        if (!hasSeen) {
          ...
          final formattedTime = timeFormat.format(session.slot1.startTime);

          // Trigger real system push notification (visible on Lock Screen & Status Bar)
          await NotificationService.showNotification(
            title: '📅 New Paper Scheduled: ${session.title}',
            body: '${session.subject} (${session.examYear}) • $formattedDate at $formattedTime. Tap to view slot details.',
            id: session.id.hashCode,
          );
        }
      }
    } catch (e) {
      debugPrint('Paper scheduled notification check error: $e');
    }
  }
